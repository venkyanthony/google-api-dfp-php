...
class PublisherQueryLanguageServiceTest extends PHPUnit_Framework_TestCase {
  ...
  protected function setUp() {
    ...
    $this->service = $this->user->GetService('PublisherQueryLanguageService', $this->version);
  }
  ...
  /**
   * Test whether we can select browsers.
   */
  public function testSelectBrowsers() {
    $selectStatement =
        new Statement('SELECT * FROM Browser LIMIT 10');
    $resultSet = $this->service->select($selectStatement);
  ...
  /**
   * Test whether we can select browser languages.
   */
  public function testSelectBrowserLanguages() {
    $selectStatement =
        new Statement('SELECT * FROM Browser_Language LIMIT 10');
    $resultSet = $this->service->select($selectStatement);
  ...
  /**
   * Test whether we can select operating systems.
   */
  public function testSelectOperatingSystems() {
    $selectStatement =
        new Statement('SELECT * FROM Operating_System LIMIT 10');
    $resultSet = $this->service->select($selectStatement);
  ...
  /**
   * Test whether we can select bandwidth groups.
   */
  public function testSelectBandwidthGroups() {
    $selectStatement =
        new Statement('SELECT * FROM Bandwidth_Group LIMIT 5');
    $resultSet = $this->service->select($selectStatement);

    $this->assertTrue(isset($resultSet));
    $this->assertGreaterThanOrEqual(1, sizeof($resultSet->columnTypes));
    $this->assertEquals(5, sizeof($resultSet->rows));
    $this->assertEquals(sizeof($resultSet->columnTypes),
        sizeof($resultSet->rows[0]->values));
  }
}
