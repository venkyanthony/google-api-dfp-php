...
try {
  ...
  $activityService = $user->GetService('ActivityService', 'v201302');
  ...
  // activities.
  $pqlTemplate = 'WHERE activityGroupId = :activityGroupId and status = ' .
      ':status ORDER BY id LIMIT %d OFFSET %d';
  $STATUS = 'ACTIVE';
  ...
  do {
    ...
    $vars = MapUtils::GetMapEntries(array('activityGroupId' =>
        new NumberValue($activityGroupId), 'status' => new TextValue($STATUS)));
