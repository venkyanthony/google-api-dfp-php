 ...
 * CreateThirdPartySlotExample.php.
 *
...
try {
  ...
  $thirdPartySlotService =
      $user->GetService('ThirdPartySlotService', 'v201203');

  // Create a statement.
  $filterStatement = new Statement("WHERE status = :status LIMIT 500",
      MapUtils::GetMapEntries(array('status' => new TextValue('ARCHIVED'))));
  ...
  if (isset($page->results)) {
    ...
    foreach ($page->results as $thirdPartySlot) {
        printf("Third party slot with ID '%s' was found.\n",
            $thirdPartySlot->id);
      $i++;
    }
  }
