<?php
/**
 * This panel lists the placements in the account as an HTML list.
 *
 * PHP version 5
 *
...
try {
  ...
  // Create service.
  $placementService = $user->GetService('PlacementService');

  // Get all placements.
  $placements = $filterText == ''
      ? ServiceUtils::GetAllObjects($placementService,
          'getPlacementsByStatement', $filterText)
      : ServiceUtils::GetSomeObjects($placementService,
          'getPlacementsByStatement', $filterText);

  if (sizeof($placements) > 0) {
    // Output HTML.
    DisplayUtils::PrintNameAndIdList($placements, 'placement');
  } else {
