...
try {
  ...
  // Create service.
  $userService = $user->GetService('UserService');

  // Get all users.
  $users = $filterText == ''
      ? ServiceUtils::GetAllObjects($userService, 'getUsersByStatement',
          $filterText)
      : ServiceUtils::GetSomeObjects($userService, 'getUsersByStatement',
          $filterText);

  if (sizeof($users) > 0) {
    // Output HTML.
    DisplayUtils::PrintNameAndIdList($users, 'user');
  } else {
